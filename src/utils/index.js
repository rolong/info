...
export function getPoolLink(token0Address, token1Address = null, remove = false) {
  if (!token1Address) {
    return (
      `https://swap.rbatscan.com/#/` +
      (remove ? `remove` : `add`) +
      `/v2/${token0Address === '0xcc24b6ca44679b453b9710dbf1b47801f3fd11af' ? 'RBAT' : token0Address}/${'RBAT'}`
    )
  } else {
    return (
      `https://swap.rbatscan.com/#/` +
      (remove ? `remove` : `add`) +
      `/v2/${token0Address === '0xcc24b6ca44679b453b9710dbf1b47801f3fd11af' ? 'RBAT' : token0Address}/${token1Address === '0xcc24b6ca44679b453b9710dbf1b47801f3fd11af' ? 'RBAT' : token1Address
      }`
    )
  }
}
...
export function getSwapLink(token0Address, token1Address = null) {
  if (!token1Address) {
    return `https://swap.rbatscan.com/#/swap?inputCurrency=${token0Address}`
  } else {
    return `https://swap.rbatscan.com/#/swap?inputCurrency=${token0Address === '0xcc24b6ca44679b453b9710dbf1b47801f3fd11af' ? 'RBAT' : token0Address
      }&outputCurrency=${token1Address === '0xcc24b6ca44679b453b9710dbf1b47801f3fd11af' ? 'RBAT' : token1Address}`
  }
}

export function getMiningPoolLink(token0Address) {
  return `https://swap.rbatscan.com/#/uni/ETH/${token0Address}`
}

export function getUniswapAppLink(linkVariable) {
  let baseUniswapUrl = 'https://swap.rbatscan.com/#/uni'
  if (!linkVariable) {
...
export const urls = {
  showTransaction: (tx) => `https://rbatscan.com/tx/${tx}/`,
  showAddress: (address) => `https://www.rbatscan.com/address/${address}/`,
  showToken: (address) => `https://www.rbatscan.com/token/${address}/`,
  showBlock: (block) => `https://rbatscan.com/block/${block}/`,
}
