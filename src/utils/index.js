...
export function getPoolLink(token0Address, token1Address = null, remove = false) {
  if (!token1Address) {
    return (
      `https://swap.opcchainpro.com/#/` +
      (remove ? `remove` : `add`) +
      `/v2/${token0Address === '0xd2a5ee27b8534da6a80fe3fc4df9a2ef225f8cb1' ? 'OPC' : token0Address}/${'OPC'}`
    )
  } else {
    return (
      `https://swap.opcchainpro.com/#/` +
      (remove ? `remove` : `add`) +
      `/v2/${token0Address === '0xd2a5ee27b8534da6a80fe3fc4df9a2ef225f8cb1' ? 'OPC' : token0Address}/${token1Address === '0xd2a5ee27b8534da6a80fe3fc4df9a2ef225f8cb1' ? 'OPC' : token1Address
      }`
    )
  }
}
...
export function getSwapLink(token0Address, token1Address = null) {
  if (!token1Address) {
    return `https://swap.opcchainpro.com/#/swap?inputCurrency=${token0Address}`
  } else {
    return `https://swap.opcchainpro.com/#/swap?inputCurrency=${token0Address === '0xd2a5ee27b8534da6a80fe3fc4df9a2ef225f8cb1' ? 'OPC' : token0Address
      }&outputCurrency=${token1Address === '0xd2a5ee27b8534da6a80fe3fc4df9a2ef225f8cb1' ? 'OPC' : token1Address}`
  }
}

export function getMiningPoolLink(token0Address) {
  return `https://swap.opcchainpro.com/#/uni/ETH/${token0Address}`
}

export function getUniswapAppLink(linkVariable) {
  let baseUniswapUrl = 'https://swap.opcchainpro.com/#/uni'
  if (!linkVariable) {
...
export const urls = {
  showTransaction: (tx) => `https://opcchainpro.com/tx/${tx}/`,
  showAddress: (address) => `https://www.opcchainpro.com/address/${address}/`,
  showToken: (address) => `https://www.opcchainpro.com/token/${address}/`,
  showBlock: (block) => `https://opcchainpro.com/block/${block}/`,
}
